...
class DocupediaPublisherApp {
    ...
    cacheElements() {
        this.el = {
            ...
            previewFrame: document.getElementById('dp-preview-frame'),
            previewContent: document.getElementById('dp-preview-content'),
            viewTabs: Array.from(document.querySelectorAll('.dp-view-tab')),
    ...
    renderReview() {
        ...
        if (!this.el.previewFrame.hidden) {
            this.renderPreview();
        }
    }
    ...
    switchView(view) {
        this.el.viewTabs.forEach((tab) => tab.classList.toggle('is-active', tab.dataset.view === view));
        if (view === 'preview') {
            this.el.previewFrame.hidden = false;
            this.el.storageXml.hidden = true;
            this.renderPreview();
        } else {
            this.el.previewFrame.hidden = true;
            this.el.storageXml.hidden = false;
        }
    }

    renderPreview() {
        const xml = (this.currentDraft ? this.currentDraft.storageXml : this.el.storageXml.value) || '';
        if (!xml.trim()) {
            this.el.previewContent.innerHTML = '<p style="color:var(--text-secondary);font-style:italic">No content to preview yet.</p>';
            return;
        }
        // Convert Confluence storage format to renderable HTML
        let html = xml;

        // Convert <ac:layout> sections into grid columns
        html = html.replace(/<ac:layout-section[^>]*ac:type="two_equal"[^>]*>/gi, '<div class="dp-preview-layout cols-2">');
        html = html.replace(/<ac:layout-section[^>]*ac:type="three_equal"[^>]*>/gi, '<div class="dp-preview-layout cols-3">');
        html = html.replace(/<ac:layout-section[^>]*>/gi, '<div class="dp-preview-layout cols-2">');
        html = html.replace(/<\/ac:layout-section>/gi, '</div>');
        html = html.replace(/<ac:layout-cell>/gi, '<div>');
        html = html.replace(/<\/ac:layout-cell>/gi, '</div>');
        html = html.replace(/<ac:layout>/gi, '');
        html = html.replace(/<\/ac:layout>/gi, '');

        // Convert <ac:image> with <ri:attachment> into attachment chip
        html = html.replace(
            /<ac:image[^>]*>\s*<ri:attachment\s+ri:filename="([^"]+)"\s*\/>\s*<\/ac:image>/gi,
            '<span class="dp-preview-attachment">$1</span>'
        );
